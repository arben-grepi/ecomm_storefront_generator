...
export async function verifyStorefrontVariantAccessibility(variantIds) {
  ...
  try {
    ...
    console.error('[Storefront API] Failed to verify variant accessibility:', error);
...
export async function createCart({
  ...
  try {
    console.log(`[Storefront API] Creating cart - Storefront: ${storefront}, Line Items: ${lineItems.length}, Market: ${market}`);
    ...
    if (userErrors && userErrors.length > 0) {
      ...
      console.error(`[Storefront API] Cart creation errors: ${errors}`);
      throw new Error(`Cart creation errors: ${errors}`);
    }

    if (!cart || !cart.checkoutUrl) {
      console.error('[Storefront API] Failed to create cart: No checkout URL returned');
    ...
    console.log(`[Storefront API] Cart created successfully - ID: ${cartId}, Checkout URL: ${cart.checkoutUrl}`);
    ...
    console.error('[Storefront API] Failed to create cart:', error.message || error);
    throw error;
  }
}
 ...
 */
export async function updateCartBuyerIdentity(cartId, shippingAddress, market = 'FI') {
  ...
  try {
    console.log(`[Storefront API] Updating cart buyer identity - Cart: ${cartGid}, Market: ${market}`);
    const data = await storefrontRequest(mutation, variables);
    const { cart, userErrors } = data.cartBuyerIdentityUpdate;

    if (userErrors && userErrors.length > 0) {
      const errors = userErrors.map(err => `${err.field}: ${err.message}`).join(', ');
      console.error(`[Storefront API] Cart buyer identity update errors: ${errors}`);
      throw new Error(`Cart buyer identity update errors: ${errors}`);
    }

    if (!cart) {
      console.error('[Storefront API] Failed to update cart buyer identity: No cart returned');
      throw new Error('Failed to update cart buyer identity: No cart returned');
    }

    console.log(`[Storefront API] Cart buyer identity updated successfully - Checkout URL: ${cart.checkoutUrl}`);
    return cart;
  } catch (error) {
    console.error('[Storefront API] Failed to update cart buyer identity:', error.message || error);
    throw error;
  }
}
 ...
 */
export async function fetchProductsForMarket(market = 'FI', first = 50) {
  const query = `
 ...
 */
export async function fetchProductForMarket(productId, market = 'FI') {
