...
export const useProductsByCategory = (categoryId, initialProducts = [], storefrontProp = null, shouldStartListeners = true) => {
  ...
  if (categoryId) {
    devLog(`[HOOK] 🎣 useProductsByCategory - Category: ${categoryId}, Initial filtered: ${initialFiltered.length}, Loading: ${loading}, Market: ${market}, Storefront: ${storefront}`);
  }
      ...
      (snapshot) => {
        const transformed = snapshot.docs
          ...
          .filter((prod) => {
            ...
            // Filter out products with less than 5 in stock (consistent with useAllProducts)
        ...
        const transformed = snapshot.docs
          ...
          .filter((prod) => {
            if (prod.marketsObject && typeof prod.marketsObject === 'object') {
              ...
              if (!prod.markets.includes(currentMarket)) return false;
            } else {
              return false;
            }
