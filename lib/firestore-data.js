 ...
 */
export const useCategories = (initialCategories = [], storefrontProp = null) => {
  ...
  useEffect(() => {
    ...
    }, [db, storefront, hasHydrated, initialCategories.length]);
 ...
 */
export const useProductsByCategory = (categoryId, initialProducts = [], storefrontProp = null) => {
  const db = getFirebaseDb();
  const storefrontFromContext = useStorefront();
  const storefront = storefrontProp || storefrontFromContext; // Use prop if provided, otherwise context
  ...
  // Memoize market - only recalculate when it actually changes
  ...
  useEffect(() => {
    ...
    console.log(`[HOOK] 🔥 useProductsByCategory: Starting Firestore listener for category: ${categoryId}, storefront: ${storefront}, market: ${market}`);
    const listenerStartTime = Date.now();
    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        ...
        console.log(`[HOOK] 📨 useProductsByCategory: Firestore snapshot received - ${totalProducts} docs (${listenerDuration}ms)`);
        
        // Performance warning for slow queries
        if (listenerDuration > 2000) {
          console.warn(`[PERF] ⚠️  useProductsByCategory query took ${listenerDuration}ms - SLOW (${totalProducts} docs)`);
        } else if (listenerDuration > 1000) {
          console.log(`[PERF] ⏱️  useProductsByCategory query took ${listenerDuration}ms (${totalProducts} docs)`);
        }
        ...
        if (filterDuration > 2000) {
          console.warn(`[PERF] ⚠️  useProductsByCategory query took ${filterDuration}ms - SLOW (${totalProducts} docs, ${transformed.length} filtered)`);
        } else if (filterDuration > 1000) {
          console.log(`[PERF] ⏱️  useProductsByCategory query took ${filterDuration}ms (${totalProducts} docs, ${transformed.length} filtered)`);
        }
    ...
    }, [db, categoryId, storefront, market, hasHydrated, initialFiltered.length]);
 ...
 */
export const useAllProducts = (initialProducts = [], storefrontProp = null) => {
  const db = getFirebaseDb();
  const storefrontFromContext = useStorefront();
  const storefront = storefrontProp || storefrontFromContext; // Use prop if provided, otherwise context
  ...
  useEffect(() => {
    ...
    }, [db, storefront, market, hasHydrated, initialProducts.length]);
