...
// priority: Higher number = preferred location for this market (local warehouses preferred over global)
export const LOCATION_MARKET_MAPPING = {
  // DSers fulfillment service - global dropship fulfillment
  // This is the fallback location when local warehouse doesn't have stock
  "114729156951": {
    name: "dsers-fulfillment-service",
    markets: ["FI", "SE", "NO", "DK", "DE", "FR", "IT", "ES", "NL", "BE", "AT", "CH", "PL", "CZ", "IE", "GB", "US", "CA", "GLOBAL"], // Can ship globally
    priority: 1, // Lower priority - use when local warehouse doesn't have stock
  },
  
  // Kosovo Warehouse - Your own warehouse in Kosovo
  // This will fulfill orders to Kosovo (XK) and Albania (AL)
  // TODO: Replace "YOUR_KOSOVO_LOCATION_ID" with actual Shopify location ID after setting up in Shopify Admin
  // To find your location ID: Shopify Admin → Settings → Locations → Click on location → Check URL or use GraphQL query
  "YOUR_KOSOVO_LOCATION_ID": {
    name: "kosovo-warehouse",
    markets: ["XK", "AL"], // Kosovo and Albania only
    priority: 2, // Higher priority - prefer local warehouse for Kosovo/Albania orders
  },
};
