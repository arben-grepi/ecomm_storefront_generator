 ...
 * @param {string} fallback - Fallback storefront if not found (default: 'FIVESTARFINDS')
 * @returns {string} Storefront code
 */
export function getStorefrontFromRequest(request, fallback = 'FIVESTARFINDS') {
  try {
 ...
 */
export async function getStorefrontFromHeaders(headers, fallback = 'FIVESTARFINDS') {
  try {
    ...
    if (pathname) {
      ...
      if (segments.length === 0) {
        return fallback; // Root path = FIVESTARFINDS
