...
let firestoreInitialized = false; // Track if we've attempted initialization
...
export const getFirebaseDb = () => {
  ...
  if (!dbInstance) {
    ...
    // Only try to initialize with persistent cache if we haven't tried before
    // This prevents errors during hot reload when Firestore is already initialized
    if (!firestoreInitialized) {
      firestoreInitialized = true;
      
      try {
        // Use the new FirestoreSettings.cache API (replaces enableIndexedDbPersistence)
        // This supports multi-tab synchronization out of the box
        ...
        if (databaseId) {
          dbInstance = initializeFirestore(app, firestoreSettings, databaseId);
        } else {
          dbInstance = initializeFirestore(app, firestoreSettings);
        }
      } catch (error) {
        // If initialization fails (e.g., already initialized with different options),
        // fall back to getFirestore which returns the existing instance
        console.warn('[Firebase] ⚠️  Failed to initialize with persistent cache, using existing instance:', error.message);
        dbInstance = databaseId ? getFirestore(app, databaseId) : getFirestore(app);
      }
    } else {
      // Already attempted initialization, just get the existing instance
      dbInstance = databaseId ? getFirestore(app, databaseId) : getFirestore(app);
    }
  }
