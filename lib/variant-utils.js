...
/**
 * Check if a string is a country name or acronym (United States, USA, US, EU, UK, AU, etc.)
 */
...
export const normalizeVariantName = (variant, productOptions = null) => {
  ...
  if (productOptions && Array.isArray(productOptions) && productOptions.length > 0) {
    ...
    for (let i = 0; i < productOptions.length; i++) {
      ...
      // Always remove country segments from beginning or end
  ...
  // Always remove country segments from fallback name if it contains "/"
