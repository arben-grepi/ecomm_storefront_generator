...
import { safeShopifyFetch } from './shopify-errors';
...
export async function adminGraphQLRequest(query, variables = {}) {
  ...
  try {
    ...
    throw new Error(`Shopify Admin GraphQL returned invalid JSON: ${error.message}`);
  }
...
export async function getProductMarkets(productGid) {
  ...
  try {
    ...
    if (!data.product) {
      ...
      return {
        publishedInFI: false, publishedInDE: false, publishedInSE: false, publishedInNO: false,
        publishedInDK: false, publishedInFR: false, publishedInIT: false, publishedInES: false,
        publishedInNL: false, publishedInBE: false, publishedInAT: false, publishedInCH: false,
        publishedInPL: false, publishedInIE: false, publishedToOnlineStore: false
      };
    }
    ...
    const markets = {
      ...
      publishedInSE: data.product.publishedInSE || false,
      publishedInNO: data.product.publishedInNO || false,
      publishedInDK: data.product.publishedInDK || false,
      publishedInFR: data.product.publishedInFR || false,
      publishedInIT: data.product.publishedInIT || false,
      publishedInES: data.product.publishedInES || false,
      publishedInNL: data.product.publishedInNL || false,
      publishedInBE: data.product.publishedInBE || false,
      publishedInAT: data.product.publishedInAT || false,
      publishedInCH: data.product.publishedInCH || false,
      publishedInPL: data.product.publishedInPL || false,
      publishedInIE: data.product.publishedInIE || false,
      publishedToOnlineStore,
    };
    
    const activeMarkets = Object.entries(markets)
      .filter(([key, value]) => key !== 'publishedToOnlineStore' && value)
      .map(([key]) => key.replace('publishedIn', ''))
      .join(', ');
    
    console.log(`[Market Sync] Product ${productId} (${data.product.title}) - Markets: ${activeMarkets || 'none'}, Online Store=${publishedToOnlineStore}`);
 ...
 */
export function buildMarketsArray(marketData) {
  const { buildMarketsArray: buildMarkets } = require('./market-utils');
  return buildMarkets(marketData);
}
