...
export const useCart = () => {
  ...
  useEffect(() => {
    devLog(`[CART] 🛒 useCart: Initializing cart`);
    let unsubscribeAuth;
    let unsubscribeCart;

    const initializeCart = async (currentUser) => {
      // Don't log email/UID for security (GDPR/compliance)
      devLog(`[CART] 🔐 Cart initialization - User: ${currentUser ? 'authenticated' : 'guest'}`);
      setUser(currentUser);

      if (currentUser) {
        // Authenticated: load from Firestore
        devLog(`[CART] 👤 Authenticated user - Loading cart from Firestore`);
        ...
        if (db) {
          ...
          // Get initial cart
          try {
            devLog(`[CART] 📥 Fetching cart from Firestore...`);
            const cartDoc = await getDoc(cartRef);
            if (cartDoc.exists()) {
              const cartData = cartDoc.data();
              const items = cartData.items || [];
              devLog(`[CART] ✅ Cart loaded from Firestore - ${items.length} item(s)`);
              setCart(items);
            } else {
              devLog(`[CART] 📦 No Firestore cart found, checking localStorage for migration...`);
              // Migrate localStorage cart to Firestore if exists
              const localCart = getLocalCart();
              if (localCart.length > 0) {
                devLog(`[CART] 🔄 Migrating ${localCart.length} item(s) from localStorage to Firestore...`);
                try {
                  ...
                  devLog(`[CART] ✅ Cart migrated to Firestore successfully`);
                  setCart(localCart);
                  localStorage.removeItem(CART_STORAGE_KEY);
                } catch (createError) {
                  console.error(`[CART] ❌ Failed to create cart in Firestore:`, createError);
                  // Fallback to localStorage if create fails
                  devLog(`[CART] ⚠️  Falling back to localStorage`);
                  setCart(localCart);
                }
              } else {
                devLog(`[CART] 📭 No cart items found (new user)`);
                setCart([]);
              }
            }
          } catch (readError) {
            ...
            const localCart = getLocalCart();
            devLog(`[CART] ⚠️  Falling back to localStorage - ${localCart.length} item(s)`);
            setCart(localCart);
          }

          // Subscribe to real-time updates
          devLog(`[CART] 👂 Subscribing to real-time cart updates...`);
          unsubscribeCart = onSnapshot(
            ...
            (snapshot) => {
              if (snapshot.exists()) {
                ...
                const items = cartData.items || [];
                devLog(`[CART] 📨 Real-time cart update received - ${items.length} item(s)`);
                setCart(items);
              } else {
                devLog(`[CART] 📭 Cart document deleted, clearing cart`);
                setCart([]);
              }
              setLoading(false);
            },
            (error) => {
              ...
              if (error.code !== 'permission-denied') {
                ...
                devLog(`[CART] ⚠️  Permission denied (expected for unauthenticated), falling back to localStorage`);
        ...
        devLog(`[CART] 👻 Guest user - Loading cart from localStorage`);
        const localCart = getLocalCart();
        devLog(`[CART] ✅ Guest cart loaded - ${localCart.length} item(s)`);
        setCart(localCart);
        setLoading(false);
      }
    };
  ...
  const saveCart = useCallback(
    async (newCart) => {
      ...
      devLog(`[CART] 💾 Saving cart - Items: ${newCart.length}, User: ${user ? 'authenticated' : 'guest'}`);
      ...
      if (user) {
        ...
        if (db) {
          ...
          try {
            ...
            devLog(`[CART] ✅ Cart saved to Firestore (${duration}ms)`);
          } catch (error) {
        ...
        if (typeof window !== 'undefined') {
          try {
            ...
            const duration = Date.now() - saveStartTime;
            devLog(`[CART] ✅ Cart saved to localStorage (${duration}ms)`);
          } catch (error) {
  ...
  const addToCart = useCallback(
    async (item) => {
      devLog(`[CART] ➕ Adding to cart - Product: ${item.productName || item.productId}, Variant: ${item.variantId}, Quantity: ${item.quantity}`);
      ...
      if (existingIndex >= 0) {
        ...
        devLog(`[CART] 🔄 Updating existing item quantity: ${oldQuantity} → ${newCart[existingIndex].quantity}`);
      } else {
        // Add new item
        newCart = [...currentCart, item];
        devLog(`[CART] ✨ Adding new item to cart`);
      }

      try {
        await saveCart(newCart);
        const duration = Date.now() - addStartTime;
        devLog(`[CART] ✅ Add to cart complete - Total items: ${newCart.length} (${duration}ms)`);
      } catch (error) {
  ...
  const removeFromCart = useCallback(
    async (productId, variantId) => {
      devLog(`[CART] ➖ Removing from cart - Product: ${productId}, Variant: ${variantId}`);
      ...
      try {
        ...
        devLog(`[CART] ✅ Remove from cart complete - Remaining items: ${newCart.length} (${duration}ms)`);
      } catch (error) {
  ...
  const updateQuantity = useCallback(
    async (productId, variantId, quantity) => {
      devLog(`[CART] 🔢 Updating quantity - Product: ${productId}, Variant: ${variantId}, New quantity: ${quantity}`);
      
      if (quantity <= 0) {
        devLog(`[CART] 🗑️  Quantity is 0, removing item instead`);
      ...
      try {
        await saveCart(newCart);
        devLog(`[CART] ✅ Quantity updated: ${oldQuantity} → ${quantity}`);
      } catch (error) {
  ...
  const clearCart = useCallback(async () => {
    devLog(`[CART] 🗑️  Clearing cart`);
    try {
      await saveCart([]);
      devLog(`[CART] ✅ Cart cleared successfully`);
    } catch (error) {
