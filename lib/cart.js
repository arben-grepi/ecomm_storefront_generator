...
export const useCart = () => {
  ...
  useEffect(() => {
    ...
    const initializeCart = async (currentUser) => {
      ...
      console.log(`[CART] 🔐 Cart initialization - User: ${currentUser ? 'authenticated' : 'guest'}`);
      setUser(currentUser);

      if (currentUser) {
        // Authenticated: load from Firestore
        console.log(`[CART] 👤 Authenticated user - Loading cart from Firestore`);
  ...
  const saveCart = useCallback(
    async (newCart) => {
      ...
      console.log(`[CART] 💾 Saving cart - Items: ${newCart.length}, User: ${user ? 'authenticated' : 'guest'}`);
