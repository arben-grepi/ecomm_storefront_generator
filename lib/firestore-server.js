...
// Fetch products by category ID (server-side) with pagination
  ...
  try {
    ...
    const products = snapshot.docs
      ...
      .filter((prod) => {
        ...
        // Filter by storefront and market
...
const getServerSideInfoCached = cache(async (language = 'en', storefront = DEFAULT_STOREFRONT) => {
  ...
  try {
    ...
    if (!hasData) {
      ...
      console.log(`[SSR] ✅ Site info fetched successfully (${duration}ms) - companyName: "${formatted.companyName || '(empty)'}", heroMainHeading: "${formatted.heroMainHeading || '(empty)'}"`);
    }
    return formatted;
  } catch (error) {
    const duration = Date.now() - startTime;
