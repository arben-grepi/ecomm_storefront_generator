...
const DEFAULT_STOREFRONT = 'FIVESTARFINDS';
...
function transformProduct(doc) {
  ...
  // Use default variant price if available (for product card), otherwise fallback to basePrice
  const displayPrice = data.defaultVariantPrice != null ? data.defaultVariantPrice : (data.basePrice || 0);
...
// Minimum stock threshold for products to be visible
const MIN_STOCK_THRESHOLD = 5;

/**
 * Check if product is available for a specific market and has sufficient stock
 * This function only checks market availability and stock - it assumes active, published, and storefront
 * filters have already been applied by the Firestore query.
 * 
 * @param {Object} prod - Product object
 * @param {string} market - Market code (e.g., 'FI', 'DE')
 * @returns {boolean} True if product is available for market and has sufficient stock
 */
function isProductAvailableForMarket(prod, market) {
  // Filter by market - support both array and object formats
  // Priority: marketsObject (new format) takes precedence over markets array (legacy)
  if (prod.marketsObject && typeof prod.marketsObject === 'object') {
    // New format: markets object { FI: { available: true, ... }, ... }
    // If marketsObject exists, it's the single source of truth (don't fall back to markets array)
    ...
    // marketData exists and available !== false, so product is available
  } else if (prod.markets && Array.isArray(prod.markets) && prod.markets.length > 0) {
    // Legacy format: markets array - must include this market
    if (!prod.markets.includes(market)) {
  ...
  // Filter out products with no in-stock variants (strict check - undefined = false)
  if (prod.hasInStockVariants !== true) {
    return false;
  }
  
  // Filter out products with insufficient stock (strict check - undefined/null = 0)
  const totalStock = typeof prod.totalStock === 'number' ? prod.totalStock : 0;
...
/**
 * Complete product visibility check (for fallback path when Firestore query doesn't filter)
 * Checks all visibility rules: active, published, storefront, market, and stock
 * 
 * @param {Object} prod - Product object
 * @param {string} storefront - Storefront code (e.g., 'LUNERA', 'HEALTH')
 * @param {string} market - Market code (e.g., 'FI', 'DE')
 * @returns {boolean} True if product passes all visibility rules
 */
function passesProductVisibilityRules(prod, storefront, market) {
  // Filter by active status
  ...
  // Filter by storefront (strict - no storefronts array = hidden)
  if (prod.storefronts && Array.isArray(prod.storefronts) && prod.storefronts.length > 0) {
    ...
    return false; // Products without storefronts array are hidden
  }
  
  // Check market availability and stock (reuse shared function)
  return isProductAvailableForMarket(prod, market);
}
...
export async function getServerSideProducts(storefront = DEFAULT_STOREFRONT, market = 'DE', limitCount = 30, startAfterDoc = null) {
  ...
  try {
    ...
    const products = snapshot.docs
      ...
      .filter((prod) => {
        // Client-side filtering for markets and stock
        // Firestore query already filters: active, publishedToOnlineStore, storefronts
        // We can't use markets array filter in query (only 1 ARRAY_CONTAINS allowed)
        // This logic MUST match validateMarketAvailability in shopify-shipping.js
        return isProductAvailableForMarket(prod, market);
      })
      .sort((a, b) => {
    ...
    try {
      ...
      const products = snapshot.docs
        .map(transformProduct)
        .filter((prod) => passesProductVisibilityRules(prod, storefront, market))
        .sort((a, b) => {
          const aViews = a.viewCount ?? a.metrics?.totalViews ?? 0;
          const bViews = b.viewCount ?? b.metrics?.totalViews ?? 0;
...
function transformProductDetail(doc) {
  ...
  return {
    ...
    defaultVariantId: data.defaultVariantId || null, // Include defaultVariantId for product detail page
...
function transformVariant(doc) {
  ...
  return {
    ...
    defaultPhoto: data.defaultPhoto || null, // Include defaultPhoto field (default photo for this variant)
...
function formatInfoData(data) {
  ...
  return {
    ...
    heroBannerTextWidth: data.heroBannerTextWidth != null ? (typeof data.heroBannerTextWidth === 'number' ? data.heroBannerTextWidth : parseFloat(data.heroBannerTextWidth) || 75) : 75,
    heroBannerCropTop: data.heroBannerCropTop != null ? (typeof data.heroBannerCropTop === 'number' ? data.heroBannerCropTop : parseFloat(data.heroBannerCropTop) || 0) : 0,
    heroBannerCropBottom: data.heroBannerCropBottom != null ? (typeof data.heroBannerCropBottom === 'number' ? data.heroBannerCropBottom : parseFloat(data.heroBannerCropBottom) || 0) : 0,
    ...
    productCardIsSquare: data.productCardIsSquare === true,
    ...
    privacyPolicySections: Array.isArray(data.privacyPolicySections) ? data.privacyPolicySections : [],
  };
}
