...
export function getAdminDb() {
  ...
  if (getApps().length === 0) {
    try {
      ...
      // - No need for explicit credentials (FIREBASE_PROJECT_ID, FIREBASE_CLIENT_EMAIL, FIREBASE_PRIVATE_KEY)
...
export async function getServerSideCategories(storefront = DEFAULT_STOREFRONT, market = 'FI') {
  ...
  try {
    ...
    // Get all available products to check which categories have products
    const products = await getServerSideProducts(storefront, market);
