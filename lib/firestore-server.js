...
import { getAdminStoreCollection, getAdminStoreDoc } from './store-collections';
...
export async function getServerSideCategories() {
  try {
    ...
    const snapshot = await getAdminStoreCollection(adminDb, 'categories')
      .orderBy('name', 'asc')
      .get();
...
export async function getServerSideProducts() {
  try {
    ...
    const snapshot = await getAdminStoreCollection(adminDb, 'products').get();
...
export async function getServerSideProductsByCategory(categoryId) {
  try {
    ...
    const snapshot = await getAdminStoreCollection(adminDb, 'products')
...
export async function getServerSideProductDetail(categorySlug, productSlug) {
  try {
    ...
    const productSnapshot = await getAdminStoreCollection(adminDb, 'products')
    ...
    const variantsSnapshot = await getAdminStoreDoc(
      adminDb,
      'products',
      productDoc.id
    )
