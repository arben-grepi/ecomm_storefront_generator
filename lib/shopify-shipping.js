...
import { getShopifyUserMessage, safeShopifyFetch } from './shopify-errors';
...
export async function getShippingRates({ lineItems, shippingAddress }) {
  ...
  try {
    ({ SHOPIFY_STORE_URL, SHOPIFY_ACCESS_TOKEN } = getShopifyCredentials());
  } catch (error) {
    return {
      available: false,
      error: getShopifyUserMessage(error, 'Shipping check is temporarily unavailable. Please try again later.'),
      rates: [],
    };
  }
  ...
  try {
    draftOrderData = {
      draft_order: {
        ...
        shipping_address: {
          ...
          country: shippingAddress.countryCode || shippingAddress.country || 'XK', // ISO 2-letter code
    ...
    return {
      available: false,
      error: getShopifyUserMessage(error, 'Unable to verify shipping. Please try again later.'),
      rates: [],
    };
  }
  ...
  try {
    ...
    const response = await safeShopifyFetch(url, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      ...
      // Return structured failure so the UI can show the message (do not throw → 500)
      console.error(`[API] ❌ getShippingRates: Shopify API error ${response.status}: ${errorText}`);
      return {
        available: false,
        error: response.status === 404
          ? 'Shipping check is temporarily unavailable. Please try again later.'
          : `Unable to verify shipping. Please try again later.`,
        rates: [],
      };
    }
    ...
    try {
      await safeShopifyFetch(`https://${SHOPIFY_STORE_URL}/admin/api/${SHOPIFY_API_VERSION}/draft_orders/${draftOrder.id}.json`, {
        method: 'DELETE',
        headers: {
          'X-Shopify-Access-Token': SHOPIFY_ACCESS_TOKEN,
        },
      }, 'Shopify draft order cleanup');
    } catch (deleteError) {
...
export async function validateInventory(lineItems, countryCode = null) {
  ...
  try {
    ...
    return {
      valid: false,
      error: getShopifyUserMessage(error, 'Inventory check is temporarily unavailable. Please try again later.'),
      unavailableItems: [],
    };
  }
  ...
  const validationResults = await Promise.all(
    lineItems.map(async (item) => {
      try {
        ...
        const variantResponse = await safeShopifyFetch(variantUrl, {
          headers: {
            'X-Shopify-Access-Token': SHOPIFY_ACCESS_TOKEN,
          },
        }, 'Shopify inventory');
        ...
        const inventoryResponse = await safeShopifyFetch(inventoryUrl, {
          headers: {
            'X-Shopify-Access-Token': SHOPIFY_ACCESS_TOKEN,
          },
...
export async function validateCheckout({ cart, shippingAddress, storefront: providedStorefront = null }) {
  ...
  const marketCountryCode = countryCode || 'XK'; // Default to XK for market/inventory checks
  ...
  // Catch Shopify/API failures so they become UI-facing validation errors, not 500s.
  const [inventoryValidation, shippingRates] = await Promise.all([
    validateInventory(lineItems, marketCountryCode).catch((error) => {
      console.error('[API] ❌ validateInventory failed:', error);
      return {
        valid: false,
        error: getShopifyUserMessage(error, 'Unable to verify inventory. Please try again later.'),
        unavailableItems: [],
      };
    }),
    countryCode
      ? getShippingRates({ lineItems, shippingAddress })
      : Promise.resolve({ available: false, error: 'Country is required for shipping validation', rates: [] }),
  ]);
