...
export function WebsiteProvider({ children }) {
  ...
  // Fetch available websites from active storefront config
  const refreshWebsites = useCallback(async () => {
    try {
      ...
      // Drop stale selection (e.g. old FIVESTARFINDS/HEALTH) if no longer active
      setSelectedWebsiteState((current) => {
        if (storefronts.includes(current)) return current;
        const next = storefronts[0] || 'LUNERA';
        if (typeof window !== 'undefined') {
          localStorage.setItem('selectedWebsite', next);
        }
        return next;
      });
    } catch (error) {
      ...
      setSelectedWebsiteState('LUNERA');
      if (typeof window !== 'undefined') {
        localStorage.setItem('selectedWebsite', 'LUNERA');
      }
    } finally {
      setLoading(false);
    }
