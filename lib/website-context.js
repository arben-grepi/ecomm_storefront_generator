...
const WebsiteContext = createContext({
  selectedWebsite: 'FIVESTARFINDS',
  availableWebsites: ['FIVESTARFINDS'],
  loading: true,
  setSelectedWebsite: () => {},
  refreshWebsites: () => {},
});
...
export function WebsiteProvider({ children }) {
  ...
  const [selectedWebsite, setSelectedWebsiteState] = useState('FIVESTARFINDS');
  const [availableWebsites, setAvailableWebsites] = useState(['FIVESTARFINDS']);
  ...
  const refreshWebsites = useCallback(async () => {
    try {
      ...
      const storefronts = data.storefronts || ['FIVESTARFINDS'];
      
      setAvailableWebsites(storefronts);
    } catch (error) {
      console.error('Failed to fetch websites:', error);
      setAvailableWebsites(['FIVESTARFINDS']); // Fallback to default
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const initialWebsite = urlWebsite || storedWebsite || 'FIVESTARFINDS';
