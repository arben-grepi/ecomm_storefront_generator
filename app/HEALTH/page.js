...
export default async function Home() {
  // 🔍 HEALTH PAGE (SERVER COMPONENT) - Set breakpoint here in Cursor
  // HEALTH storefront at /HEALTH
  // Always use English - language functionality removed
  const language = 'en';
  
  console.log(`[SSR] 🚀 Initializing Health Home page (Server Component)`);
  const pageStartTime = Date.now();
  
  const headersList = headers();
  // For pages in app/HEALTH/, always use 'HEALTH' (folder name determines storefront)
